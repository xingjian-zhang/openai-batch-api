...
from asyncio import (FIRST_COMPLETED, CancelledError, Event, Lock, Queue,
                     QueueEmpty, TimeoutError, create_task, gather, sleep,
                     wait, wait_for)
...
class AzureConfig(SimpleNamespace):
    ...
    def __init__(
            ...
            api_key: str = environ.get("AZURE_OPENAI_KEY"),
    ):
    ...
    async def __worker(self, i):
        while True:
            async with self.__lock:
                ...
                self.__progress.main.set_description(f"🟢 RUNNING",
                                                     refresh=True)
                ...
                while self.__current.tpm + avg_tpr >= effective_tpm and not self.__stopped.is_set(
                    ...
                    self.__progress.main.set_description(f"🟡 WAITING",
                                                         refresh=True)
